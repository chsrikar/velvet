...
const defaultStartupData = [
  {
    id: 1, name: 'ArchConnect', emoji: '🏗️', tagline: 'Connecting Architects & Clients',
  ...
  {
    id: 3, name: 'Eterna', emoji: '💎', tagline: 'Timeless Fashion E-Commerce',
...
function recalcProgress(milestones) {
...
export default function Startups() {
  ...
  const [newStartup, setNewStartup] = useState({ name: '', tagline: '', description: '', emoji: '🚀' })
  ...
  const toggleMilestone = (startupId, milestoneId) => {
    setStartups(prev => prev.map(s => {
      ...
      const updated = s.milestones.map(m => m.id === milestoneId ? { ...m, done: !m.done } : m)
      return { ...s, milestones: updated, progress: recalcProgress(updated) }
    }))
  }
  ...
  const addMilestone = (startupId) => {
    ...
    setStartups(prev => prev.map(s => {
      if (s.id !== startupId) return s
      const updated = [...s.milestones, { id: Date.now(), text, done: false }]
      return { ...s, milestones: updated, progress: recalcProgress(updated) }
    }))
  ...
  const addStartup = () => {
    ...
    setStartups(prev => [...prev, {
      id: Date.now(),
      name: newStartup.name.trim(),
      emoji: newStartup.emoji,
      tagline: newStartup.tagline.trim() || 'New Venture',
      description: newStartup.description.trim() || 'An exciting new startup project.',
      progress: 0,
      color: colorOptions[Math.floor(Math.random() * colorOptions.length)],
      milestones: [],
    }])
    setNewStartup({ name: '', tagline: '', description: '', emoji: '🚀' })
    ...
    <div className="page-enter">
      ...
      <div className="flex items-center justify-end mb-4">
        <button
          ...
          className={`text-xs px-4 py-2 rounded-full border-none cursor-pointer transition-all duration-300 ${
            darkMode
              ? 'bg-white/10 text-white/60 hover:bg-white/20'
              : 'bg-white/50 text-dark1/60 hover:bg-white/80'
          }`}
        >
          {showAddStartup ? 'Cancel' : '+ Add Startup'}
        </button>
      </div>

      {/* Add Startup Form */}
      {showAddStartup && (
        <div className={`rounded-3xl p-6 mb-6 animate-fade-in ${
          darkMode ? 'bg-white/5 border border-white/5' : 'bg-white/40 border border-white/30'
        }`}>
          <h4 className="font-display text-base font-semibold mb-3">New Startup</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
            <div className="flex gap-2">
              <select
                value={newStartup.emoji}
                onChange={(e) => setNewStartup(prev => ({ ...prev, emoji: e.target.value }))}
            ...
            <input
              ...
              className={`text-sm ${darkMode ? 'input-field-dark' : 'input-field'}`}
            />
          </div>
          <input
            type="text"
            ...
            className={`text-sm mb-3 ${darkMode ? 'input-field-dark' : 'input-field'}`}
          />
          <button onClick={addStartup} className="btn-primary text-sm">Create Startup</button>
        </div>
      )}
      ...
      <div className="space-y-6">
        {startups.map((startup) => (
          <div
            key={startup.id}
            className={`rounded-3xl p-6 md:p-8 transition-all duration-300 card-hover group relative ${
              darkMode
          ...
          >
            ...
            <button
              ...
              className="absolute top-4 right-4 opacity-0 group-hover:opacity-60 text-sm cursor-pointer border-none bg-transparent hover:opacity-100 transition-all z-10"
            >
              ✕
            </button>
            ...
            <div className="flex flex-col md:flex-row md:items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <div className={`w-full h-2.5 rounded-full mb-4 overflow-hidden ${darkMode ? 'bg-white/10' : 'bg-white/50'}`}>
                  <div
                ...
                <button
                  ...
                  className={`text-sm font-body cursor-pointer border-none bg-transparent ${darkMode ? 'text-lavender' : 'text-dark3'}`}
                >
                ...
                {expandedId === startup.id && (
                  <div className="mt-3 space-y-2 animate-fade-in">
                    {startup.milestones.map((m) => (
                      <div
                        key={m.id}
                        className={`flex items-center gap-3 px-3 py-2 rounded-xl group/ms ${darkMode ? 'bg-white/5' : 'bg-white/40'}`}
                      >
                        <div
                        ...
                        <button
                          ...
                          className="opacity-0 group-hover/ms:opacity-60 text-xs cursor-pointer border-none bg-transparent hover:opacity-100 transition-all"
                        >
                          ✕
                        </button>
                      </div>
                    ...
                    <div className="flex gap-2 mt-2">
                      <input
                        ...
                        className={`flex-1 text-sm ${darkMode ? 'input-field-dark' : 'input-field'}`}
                      />
                      <button onClick={() => addMilestone(startup.id)} className="btn-primary text-sm !px-4 !py-2">Add</button>
                    </div>
                  </div>
                )}
              </div>
